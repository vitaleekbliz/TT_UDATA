...
from app.api.routers.models.models_lots_endpoints import LotResponse, BidRequest
from typing import List

lot_endpoint_logger = AppLogger("LOTS_ENDPOINT", "lots_endpoint.log", app_logger_settings.LEVEL).get_instance()
router = APIRouter(prefix="/lots", tags=["Lots"])

# Отримуємо екземпляр менеджера (Singleton)
def get_manager():
    return BidManager() 
...
async def create_lot(manager: BidManager = Depends(get_manager)):
    try:
        lot_id = await manager.create_lot(
        ...
        lot_endpoint_logger.info(f"API: Created lot {lot_id}")
        return {"message" : f"API: Created lot!", "lot_id" : lot_id}
...
async def list_active_lots(manager: BidManager = Depends(get_manager)):
    # Використовуємо ваш новий метод з list comprehension
    try:
...
async def place_bid(lot_id: int, bid_data: BidRequest, manager: BidManager = Depends(get_manager)):
    try:
        manager.bid_on_lot(lot_id, bid_data.amount)
        return {"message": "Bid placed successfully"}
    except Exception as e:
        # Тут ми ловимо ваші AuctionEndedError або BidTooLowError
